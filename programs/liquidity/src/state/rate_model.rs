...
mod tests {
    ...
    // =========================================================================
    // [M-2] MEDIUM — Rate Model V1 Allows Economically Perverse Declining Rate
    //              Before Kink (Missing Monotonicity Check)
    //
    // Audit finding: `update_rate_data_v1` does NOT require
    // `rate_at_utilization_zero <= rate_at_utilization_kink`.
    // An admin can configure a rate curve where the borrow rate *decreases* as
    // utilization increases from 0% to the kink.  This is economically perverse
    // because it incentivises over-utilisation: borrowers pay *less* as more of
    // the reserve is borrowed.
    //
    // The validation code only checks `rate_at_kink <= rate_at_max` (declining
    // rate after the kink is rejected) but has no symmetric check before it.
    // Specifically, `set_rate_v1` contains:
    //
    //   if rate_data.kink == 0 ||
    //      rate_data.kink >= FOUR_DECIMALS ||
    //      rate_data.rate_at_utilization_kink > rate_data.rate_at_utilization_max
    //   { return Err(ErrorCodes::InvalidParams.into()); }
    //
    // There is no `rate_data.rate_at_utilization_zero > rate_data.rate_at_utilization_kink`
    // guard here.
    //
    // Impact: a misconfigured (or maliciously configured) rate model could make
    // high-utilisation borrowing cheaper than low-utilisation borrowing, draining
    // the reserve while rewarding the behaviour the protocol needs to discourage.
    // =========================================================================

    /// [M-2] PoC #1 — Declining rate before kink is ACCEPTED by `set_rate_v1`.
    ///
    /// Configures `rate_at_zero = 5000` (50%) and `rate_at_kink = 100` (1%).
    /// The validation must NOT return an error — demonstrating the vulnerability.
    #[test]
    fn test_m2_v1_declining_rate_before_kink_is_accepted() {
        let mut rate_model = RateModel::default();

        // rate_at_zero (50%) > rate_at_kink (1%): perverse declining slope
        let rate_data = RateDataV1Params {
            kink: 8000,                      // 80% utilization kink
            rate_at_utilization_zero: 5000,  // 50% — HIGH at 0% utilization
            rate_at_utilization_kink: 100,   // 1%  — LOW at kink (perverse!)
            rate_at_utilization_max: 15000,  // 150% — HIGH again after kink
        };

        // [M-2] VULNERABILITY PROVEN: set_rate_v1 must NOT reject this config.
        // The missing monotonicity check allows rate_at_zero > rate_at_kink.
        let result = rate_model.set_rate_v1(rate_data);
        assert!(
            result.is_ok(),
            "[M-2] VULNERABILITY NOT PRESENT — declining rate before kink was rejected. \
             If this assertion fires, the bug has been fixed."
        );

        println!(
            "\n[M-2 PROVEN] set_rate_v1 accepted rate_at_zero=50% > rate_at_kink=1%.\n\
             A monotonically DECREASING rate curve from 0% to 80% utilisation is now live.\n\
             Borrowers pay LESS as utilisation increases toward the kink — a perverse incentive\n\
             that rewards over-utilisation and can silently drain the reserve."
        );
    }

    /// [M-2] PoC #2 — The resulting rate curve is DECREASING before the kink.
    ///
    /// After setting a perverse config, calc_borrow_rate_from_utilization returns
    /// LOWER rates as utilisation increases from 0% toward the kink.
    #[test]
    fn test_m2_v1_rate_decreases_before_kink() {
        let mut rate_model = RateModel::default();
        ...
        let rate_at_0_pct = rate_model.calc_borrow_rate_from_utilization(0).unwrap();
        let rate_at_40_pct = rate_model.calc_borrow_rate_from_utilization(4000).unwrap();
        let rate_at_80_pct = rate_model.calc_borrow_rate_from_utilization(8000).unwrap();

        // [M-2] The rate curve must decrease as utilisation increases to the kink.
        assert!(
            rate_at_0_pct > rate_at_40_pct,
            "[M-2] Rate at 0% ({}) should be GREATER than rate at 40% ({}) — \
             confirming the declining curve",
            rate_at_0_pct, rate_at_40_pct
        );
        assert!(
            rate_at_40_pct > rate_at_80_pct,
            "[M-2] Rate at 40% ({}) should be GREATER than rate at kink 80% ({}) — \
             confirming the declining curve",
            rate_at_40_pct, rate_at_80_pct
        );

        println!(
            "\n[M-2 PROVEN] Borrow rate curve DECREASES before the kink:\n\
             Utilisation  0% → rate = {} ({}%)\n\
             Utilisation 40% → rate = {} ({}%)\n\
             Utilisation 80% → rate = {} ({}%) ← kink\n\
             As utilisation rises from 0% to 80%, borrowers pay LESS interest.\n\
             This incentivises maximum borrowing up to the kink.",
            rate_at_0_pct,  rate_at_0_pct  / 100,
            rate_at_40_pct, rate_at_40_pct / 100,
            rate_at_80_pct, rate_at_80_pct / 100,
        );
    }

    /// [M-2] PoC #3 — Control: declining rate AFTER kink is still rejected.
    ///
    /// Confirms the existing check works in the other direction, making the
    /// missing pre-kink monotonicity check the precise point of failure.
    #[test]
    ...
    /// [M-2] PoC #4 — Edge case: rate_at_zero == rate_at_kink is allowed (flat slope).
    ///
    /// A flat slope before the kink is a degenerate but technically valid curve.
    /// This is accepted by the existing code (as it should be), and should remain
    /// accepted.  The vulnerability is specifically that rate_at_zero can be
    /// STRICTLY GREATER than rate_at_kink.
    #[test]
